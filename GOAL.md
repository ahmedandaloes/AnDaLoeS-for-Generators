...
- [~] Localized: Profile + Home + Generator Detail (call/share/copy/save/report/Rent Now, acceptance+on-time stats via placeholders, price estimate) with real Arabic; RTL paddings→Directional. Next: owner dashboard → admin → rest. Search hint + sort labels + section titles + documents pending. Search hint + sort labels + section titles pending. Search hint + sort labels + section titles pending.
