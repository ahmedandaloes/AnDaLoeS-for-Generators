...
- [~] Localized: Profile + Home (nav tooltips, sort sheet, verified badge, empty/error/no-match states) with real Arabic; RTL paddings → EdgeInsetsDirectional. Next: generator detail → booking → my rentals → owner dashboard → admin. Search hint + sort labels pending.
